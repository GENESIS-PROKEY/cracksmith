...
#  [License]
#
#  all rights are with me no one can copy the code or change the code
...
__author__ = "CrackSmith Contributors"
__license__ = "MIT"
__version__ = "1.0.0"
...
╚██████╗██║  ██║██║  ██║╚██████╗██║  ██╗███████║██║ ╚═╝ ██║██║   ██║   ██║  ██║
 ╚═════╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝╚═╝   ╚═╝   ╚═╝  ╚═╝
    ...
    console.print("[bold green]Advanced Password Wordlist Generator[/bold green]")
    console.print(f"[yellow]Version: {__version__}[/yellow]\n")


def version():
    """Display version"""

    print("\r\n	\033[1;31m[ CrackSmith.py ]  " + __version__ + "\033[1;m\r\n")
...
def improve_dictionary(file_to_open):
    ...
    if leetmode == "y":
        for (
            ...
            unique_lista
        ):  # if you want to add more leet chars, you will need to add more lines in cracksmith.cfg too...
    ...
    print_to_file(file_to_open + ".new.txt", unique_list_finished)
...
@click.option('-i', '--interactive', is_flag=True, help='Interactive questions for user password profiling')
@click.option('-q', '--quiet', is_flag=True, help="Quiet mode (don't print banner)")
@click.version_option(__version__, '-v', '--version', help='Show the version of this program.')
@click.pass_context
def main(ctx, interactive, quiet):
    """
    CrackSmith: Advanced Password Wordlist Generator
    """
    read_config(os.path.join(os.path.dirname(os.path.realpath(__file__)), "cracksmith.cfg"))
    console = Console()
...
def improve(filename):
    """Improve an existing dictionary."""
    improve_dictionary(filename)

@main.command()
def download():
    """Download huge wordlists from a repository."""
    download_wordlist()

@main.command()
def alecto():
    """Parse default usernames and passwords from Alecto DB."""
    alectodb_download()
